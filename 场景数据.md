...
1.  **Thought**: 检测到输入为 URL。判定为“精准检索模式”。
    *   **URL Deconstruction**: 必须显式提取以下信息：
        *   **Domain**: 链接所属平台（如 youtube.com, bilibili.com, mp.weixin.qq.com 等）。
        *   **ID**: 视频或文章的唯一标识符（如 `v=xxxx` 或 `BVxxxx`）。
        *   **Title/Keywords**: 如果 URL 中包含标题（如知乎专栏）或可推测的主题，提取之。
    *   **Strategy**: 单纯搜索 URL 往往得不到详细内容。必须构建组合拳：
        *   Query 1: 原始 URL (Direct Access)
        *   Query 2: `site:[domain] [video_id/article_id]` (强制站内精准定位)
        *   Query 3: `[video_id] "summary" OR "transcript" OR "字幕"` (尝试获取内容摘要或脚本)
        *   Query 4: `[Title/Keywords] site:[domain]` (通过标题反查)

2.  **Notification**: "检测到具体的资源链接。我正在通过**[提取的域名/标题]**进行定向检索。为了确保获取完整内容，我将同时检索其**字幕、摘要及站内元数据**，力求还原核心观点..."
...
1.  **Single Retrieval Only**: 为了性能优化，**全程只允许调用一次 `get_datas` 工具**。你必须在这一次调用中填入最具有代表性的4个 `web_queries`。严禁分多次调用或循环调用。
...
  "methods_web_video_nums": 0,
  "methods_web_domain_select": "web",
  "methods_web_data_nums": "2"
}
```
